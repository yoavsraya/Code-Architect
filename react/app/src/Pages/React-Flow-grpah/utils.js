...
export function createNodesAndEdges(vertices, edges) {
    const xSpacing = 150;
    const ySpacing = 100;
    const folderWidth = 300;
    const folderHeight = 400;
    const nodePadding = 30;
    const maxNodesPerRow = vertices.length / 3;
    ...
    Object.keys(groupedNodes).forEach((folderIndex, folderIdx) => {
      ...
      const folderStartX = folderIdx * (folderWidth + xSpacing);
      ...
      nodesInFolder.forEach((vertex, index) => {
        ...
        const x = folderStartX + column * (xSpacing + nodePadding);
        const y = folderStartY + row * (ySpacing + nodePadding);
